...
class TeddyController extends FlareControls {
  ...
  final _globalToFlareWorld = Mat2D();

  // Caret in Flutter global coordinates.
  final _caretGlobal = Vec2D();

  // Caret in Flare world coordinates.
  final _caretWorld = Vec2D();

  // Store the origin in both world and local transform spaces.
  final _faceOrigin = Vec2D();
  final _faceOriginLocal = Vec2D();
  ...
  bool advance(FlutterActorArtboard artboard, double elapsed) {
    ...
    if (_hasFocus) {
      ...
      final toCaret = Vec2D.subtract(Vec2D(), _caretWorld, _faceOrigin);
  ...
  void coverEyes({bool shoudCoverEyes = false}) {
    if (_isCoveringEyes == shoudCoverEyes) {
      return;
    }
    _isCoveringEyes = shoudCoverEyes;
    if (shoudCoverEyes) {
      play('hands_up');
    } else {
      play('hands_down');
